...
export function FinanceAdminWorkspace({data,onRun}){
  ...
  const run=async(action,input)=>{if(!onRun)return;setBusy(true);try{return await onRun(action,input)}finally{setBusy(false)}};
  ...
  const openSettlement=item=>setSettlementDraft({titleId:item.id,amount:moneyInput(item.openAmountMinor),accountId:item.accountId??'',method:'pix'});
  const confirmSettlement=async()=>{if(!settlementDraft)return;await run('settleTitle',{id:uid('settlement'),operationId:uid('settlement-op'),titleId:settlementDraft.titleId,amountMinor:moneyMinor(settlementDraft.amount),occurredAt:now(),accountId:settlementDraft.accountId||null,method:settlementDraft.method||null});setSettlementDraft(null)};
  const cancelTitle=async item=>run('cancelTitle',{id:item.id,cancelledAt:now(),reason:'Cancelamento manual'});
  const reverseSettlement=async item=>run('reverseSettlement',{id:uid('reversal'),operationId:uid('reversal-op'),settlementId:item.id,occurredAt:now(),reason:'Estorno manual'});
  const reconcile=async(line,mode)=>{const draft=reconcileDraft[line.id]??{};await run('reconcileStatement',mode==='existing'?{rowId:line.id,settlementId:draft.settlementId}:{rowId:line.id,mode:'adjustment',operationId:uid('reconciliation-op'),accountId:draft.accountId||null,categoryId:draft.categoryId||null,description:line.description,notes:'Ajuste explícito pela conciliação local'});setReconcileDraft(current=>({...current,[line.id]:{}}))};
  const patchReconcile=(lineId,changes)=>setReconcileDraft(current=>({...current,[lineId]:{...(current[lineId]??{}),...changes}}));
  const titleRows=items=>items.length?items.slice(0,12).map(item=><div className="finance-title-row" key={item.id}><div><strong>{item.description}</strong><small>{date(item.dueAt)} · {statusLabel[item.status]??item.status}</small></div><span>{money(item.openAmountMinor)}</span><div className="actions">{!['settled','cancelled'].includes(item.status)&&<button type="button" onClick={()=>openSettlement(item)}>Baixar</button>}{!['settled','cancelled'].includes(item.status)&&<button type="button" className="ghost" disabled={busy} onClick={()=>void cancelTitle(item)}>Cancelar</button>}</div></div>):<p className="finance-admin-empty">Nenhum título.</p>;
  ...
  return <section className="finance-admin-workspace" data-testid="finance-admin-workspace">
    ...
    <section className="finance-admin-card" data-testid="finance-context-reverse">
      ...
      <div className="finance-title-list">{activeSettlements.slice(0,10).map(item=><div className="finance-title-row" key={item.id}><div><strong>{titleById.get(item.titleId)?.description??'Título'}</strong><small>{date(item.occurredAt)} · {item.method??'Sem forma informada'}</small></div><span>{money(item.amountMinor)}</span><button type="button" disabled={busy} onClick={()=>void reverseSettlement(item)}>Estornar</button></div>)}{!activeSettlements.length&&<p className="finance-admin-empty">Nenhuma baixa ativa para estornar.</p>}</div>
    </section>

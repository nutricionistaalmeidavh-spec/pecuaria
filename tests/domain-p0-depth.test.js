...
test('commercial settlement distinguishes live arrobas from carcass arrobas',()=>{
  ...
  const result=finance.calculateCattleSettlement({
    liveWeightKg:450,
    carcassYieldPct:50,
    pricePerCarcassArrobaMinor:30000,
...
test('reproduction supports pregnancy loss and derives breeding KPIs',()=>{
  ...
  const events=[
    domain.recordReproductionEvent({id:'s1',animalId:'a1',type:'service',occurredAt:'2026-01-01'}),
    domain.recordReproductionEvent({id:'s2',animalId:'a2',type:'service',occurredAt:'2026-01-01'}),
    domain.recordReproductionEvent({id:'p1',animalId:'a1',type:'pregnancy-check',occurredAt:'2026-02-01',metadata:{result:'positive'}}),
    domain.recordReproductionEvent({id:'p2',animalId:'a2',type:'pregnancy-check',occurredAt:'2026-02-01',metadata:{result:'negative'}}),
    domain.recordReproductionEvent({id:'l1',animalId:'a1',type:'pregnancy-loss',occurredAt:'2026-03-01'})
  ];

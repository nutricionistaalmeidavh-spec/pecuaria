...
import {P2FieldFavorites,P2LocalTelemetryPanel} from './p2-ux.jsx';
...
export function P0CorralFlow({animals=[],onRecord}){
  ...
  return <section className="panel p2-corral-density" data-testid="corral-flow"><div className="panel-heading"><div><span className="eyebrow">Curral</span><h2>Pesagem rápida</h2><p>Identifique por brinco/RFID, confira o último peso e registre sem tirar o foco do fluxo.</p></div></div>
...
export function FieldP0CommandBar({syncState}){
  ...
  return <section className="panel p0-compact-panel"><div className="panel-heading"><div><span className="eyebrow">Atalhos de campo</span><h2>Manejo por intenção</h2><p>As ações mais usadas ficam agrupadas sem alterar o protocolo offline existente.</p></div></div><P2FieldFavorites onOpen={open}/><div className="field-sync-summary" data-testid="field-sync-status"><div><span>Dispositivo</span><strong>{syncState?.paired?'Pareado':'Não pareado'}</strong></div><div><span>Na fila</span><strong>{syncState?.pending??0}</strong></div><div><span>Conflitos</span><strong>{syncState?.conflicts??0}</strong></div></div><div className="field-action-groups" data-testid="field-action-groups">{groups.map(([group,actions])=><div className="field-action-group" key={group}><small>{group}</small>{actions.map(([label,target])=><button type="button" key={label} onClick={()=>open(target)}>{label}</button>)}</div>)}</div></section>;
}

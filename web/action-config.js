...
export const ACTION_FORMS=Object.freeze({
  ...
  sanitary:Object.freeze({
    ...
    batchRecord:form('Aplicação sanitária coletiva',[field('animalIds','Animais','list',{refCollection:'animals'}),field('protocolId','Protocolo','text',{refCollection:'protocols'}),field('productItemId','Produto/insumo'),field('dose','Dose','number',{step:'0.01'}),field('unit','Unidade'),field('occurredAt','Aplicação','datetime-local'),field('nextDueAt','Próxima dose','datetime-local')],v=>({...v,animalIds:list(v.animalIds),protocolId:clean(v.protocolId),productItemId:clean(v.productItemId),dose:num(v.dose),unit:clean(v.unit),occurredAt:dateTime(v.occurredAt),nextDueAt:v.nextDueAt?dateTime(v.nextDueAt):null})),
  ...
  reproduction:Object.freeze({
    batchRecord:form('Reprodução coletiva',[field('animalIds','Animais','list',{refCollection:'animals'}),field('type','Evento','select',{options:[['service','Cobertura/IA'],['pregnancy-check','Diagnóstico de gestação'],['weaning','Desmame']]}),field('occurredAt','Data/hora','datetime-local')],v=>({animalIds:list(v.animalIds),type:v.type,occurredAt:dateTime(v.occurredAt)})),
    record:form('Registrar reprodução',[field('id','ID do evento'),field('animalId','Animal','text',{refCollection:'animals'}),field('type','Evento','select',{options:[['service','Cobertura/IA/IATF'],['pregnancy-check','Diagnóstico de gestação'],['calving','Parto'],['weaning','Desmame']]}),field('occurredAt','Data/hora','datetime-local'),field('relatedAnimalId','Animal relacionado','text',{refCollection:'animals'}),field('method','Método'),field('bullOrSemen','Touro / sêmen'),field('result','Resultado'),field('expectedCalvingAt','Previsão de parto','datetime-local'),field('notes','Observações')],v=>({id:clean(v.id),animalId:clean(v.animalId),type:v.type,occurredAt:dateTime(v.occurredAt),relatedAnimalId:clean(v.relatedAnimalId)||null,metadata:{method:clean(v.method)||null,bullOrSemen:clean(v.bullOrSemen)||null,result:clean(v.result)||null,expectedCalvingAt:v.expectedCalvingAt?dateTime(v.expectedCalvingAt):null,notes:clean(v.notes)||null}}))}),
  ...
  reports:Object.freeze({
    csv:form('Gerar CSV',[field('type','Relatório','select',{options:[['animal-history','Histórico do animal'],['lot-kpis','Indicadores do lote'],['sanitary','Manejo sanitário']]}),field('animalId','Animal (para histórico)','text',{refCollection:'animals'}),field('lotId','Lote (opcional)','text',{refCollection:'lots'})],v=>({type:v.type,animalId:clean(v.animalId)||undefined,lotId:clean(v.lotId)||undefined})),
    issue:form('Emitir documento',[field('id','ID'),field('type','Tipo','select',{options:[['animal-history','Histórico do animal'],['lot-kpis','Indicadores do lote'],['sanitary','Manejo sanitário']]}),field('format','Formato','select',{options:[['csv','CSV'],['pdf','PDF']]}),field('animalId','Animal (para histórico)'),field('lotId','Lote (opcional)','text',{refCollection:'lots'})],v=>({id:clean(v.id),type:v.type,format:v.format,animalId:clean(v.animalId)||undefined,lotId:clean(v.lotId)||undefined}))
  }),
  traceability:Object.freeze({
    save:form('Registrar rastreabilidade',[field('id','ID'),field('animalId','Animal','text',{refCollection:'animals'}),field('officialId','Identificação oficial / SISBOV'),field('type','Tipo','select',{options:[['identity','Identificação'],['gta','GTA'],['sisbov','SISBOV'],['certificate','Certificado']]}),field('documentNumber','Número do documento'),field('issuer','Emissor'),field('issuedAt','Emissão','datetime-local'),field('expiresAt','Validade','datetime-local'),field('notes','Observações')],v=>({...v,animalId:clean(v.animalId),issuedAt:dateTime(v.issuedAt),expiresAt:v.expiresAt?dateTime(v.expiresAt):null})),
    remove:form('Excluir registro de rastreabilidade',[field('id','ID'),field('expectedVersion','Versão','number')],v=>({id:clean(v.id),expectedVersion:num(v.expectedVersion)}))
  }),
  inventory:Object.freeze({
    save:form('Cadastrar insumo',[field('id','ID'),field('name','Nome'),field('kind','Tipo','select',{options:[['medicine','Medicamento'],['vaccine','Vacina'],['supplement','Suplemento'],['feed','Alimento'],['other','Outro']]}),field('unit','Unidade'),field('quantity','Quantidade','number',{step:'0.01'}),field('minQuantity','Estoque mínimo','number',{step:'0.01'}),field('batch','Lote/partida'),field('expiresAt','Validade','datetime-local'),field('costMinor','Custo unitário (centavos)','number')],v=>({...v,quantity:num(v.quantity),minQuantity:num(v.minQuantity),costMinor:num(v.costMinor),expiresAt:v.expiresAt?dateTime(v.expiresAt):null})),
    adjust:form('Movimentar estoque',[field('id','Insumo'),field('delta','Entrada (+) ou saída (-)','number',{step:'0.01'})],v=>({id:clean(v.id),delta:num(v.delta)}))
  }),
  pastures:Object.freeze({save:form('Cadastrar pastagem/área',[field('id','ID'),field('name','Nome'),field('farmUnitId','Fazenda','text',{refCollection:'farms'}),field('areaHa','Área (ha)','number',{step:'0.01'}),field('capacityAu','Capacidade (UA)','number',{step:'0.01'}),field('forage','Forrageira'),field('status','Status','select',{options:[['active','Ativa'],['resting','Descanso'],['maintenance','Manutenção']]})],v=>({...v,areaHa:num(v.areaHa),capacityAu:v.capacityAu===''?null:num(v.capacityAu)}))}),
  nutrition:Object.freeze({save:form('Plano nutricional',[field('id','ID'),field('name','Nome'),field('lotId','Lote','text',{refCollection:'lots'}),field('feedItemId','Insumo/alimento'),field('dailyKgPerHead','kg/cabeça/dia','number',{step:'0.01'}),field('startsAt','Início','datetime-local'),field('endsAt','Fim','datetime-local'),field('notes','Observações')],v=>({...v,lotId:clean(v.lotId),dailyKgPerHead:num(v.dailyKgPerHead),startsAt:dateTime(v.startsAt),endsAt:v.endsAt?dateTime(v.endsAt):null}))}),
  tasks:Object.freeze({
    save:form('Agendar manejo',[field('id','ID'),field('title','Tarefa'),field('dueAt','Prazo','datetime-local'),field('kind','Tipo','select',{options:[['management','Manejo'],['sanitary','Sanidade'],['reproduction','Reprodução'],['weighing','Pesagem'],['pasture','Pastagem']]}),field('animalId','Animal','text',{refCollection:'animals'}),field('lotId','Lote','text',{refCollection:'lots'}),field('notes','Observações')],v=>({...v,dueAt:dateTime(v.dueAt),animalId:clean(v.animalId)||null,lotId:clean(v.lotId)||null})),
    complete:form('Concluir tarefa',[field('id','ID da tarefa')],v=>({id:clean(v.id)}))
  }),
  data:Object.freeze({
  ...
  iot:Object.freeze({
    ...
    bindRfid:form('Vincular RFID a animal',[field('tagId','Código RFID'),field('animalId','Animal','text',{refCollection:'animals'})],v=>({tagId:clean(v.tagId),animalId:clean(v.animalId)})),

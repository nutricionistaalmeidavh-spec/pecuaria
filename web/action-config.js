...
export const ACTION_FORMS=Object.freeze({
  ...
  animals:Object.freeze({
    save:form('Salvar animal',[field('id','ID'),field('tag','Brinco/identificação'),field('name','Nome'),field('officialId','Identificação oficial / SISBOV'),field('rfid','RFID'),field('farmUnitId','Unidade/Fazenda'),field('lotId','Lote'),field('breedId','Raça'),field('categoryId','Categoria'),field('birthDate','Nascimento','date'),field('damId','Mãe'),field('sireId','Pai'),field('origin','Origem'),field('purpose','Finalidade','select',{options:[['beef','Corte'],['dairy','Leite'],['breeding','Reprodução']]}),field('sex','Sexo','select',{options:[['female','Fêmea'],['male','Macho'],['unknown','Não informado']]})]),
    batchMove:form('Mover animais em lote',[field('animalIds','Animais','list',{placeholder:'brincos/IDs, um por linha'}),field('toLotId','Lote destino'),field('movedAt','Data/hora','datetime-local'),field('reason','Motivo')],v=>({animalIds:list(v.animalIds),toLotId:clean(v.toLotId),movedAt:dateTime(v.movedAt),reason:clean(v.reason)||'management'})),
    batchLifecycle:form('Baixa coletiva',[field('animalIds','Animais','list'),field('type','Evento','select',{options:[['death','Morte'],['sale','Venda'],['disposal','Baixa']]}),field('occurredAt','Data/hora','datetime-local'),field('reason','Motivo')],v=>({animalIds:list(v.animalIds),type:v.type,occurredAt:dateTime(v.occurredAt),reason:clean(v.reason)||null})),

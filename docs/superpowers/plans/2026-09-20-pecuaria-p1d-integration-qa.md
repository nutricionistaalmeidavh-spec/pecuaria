...
**Architecture:** Este bloco não cria um quarto subsistema. Ele fecha lacunas de integração entre financeiro, pastagens e campo, fortalece os gates existentes e atualiza a documentação. `qa/product-contract.json` permanece a fonte contratual da superfície; `qa-phase5` deve continuar fail-closed; GitHub Actions é a evidência final antes de merge.
...
- Modify: `tooling/qa-phase5.mjs` only if evidence does not already expose the final counts
...
- [ ] If `qa-phase5` already computes counts dynamically, do not refactor it. If evidence omits final action/RPC counts, add them without changing equality semantics.
...
- Modify: `tooling/compatibility-check.mjs` only if the current generic check cannot assert the new collections safely
...
- [ ] Run `node --test tests/p1-backward-compatibility.test.js tests/backup-integrity.test.js` plus `npm run compat:contract`.
...
- Modify: `tooling/security-gate.mjs` only when a missing invariant cannot be expressed by existing test discovery

- [ ] Add role matrix tests for admin/manager/finance/field-operator/viewer across finance mutations, pasture writes and field quick operations.
- [ ] Prove `fieldSync.quick` authorizes the authenticated actor and destination action; a field operator may perform cattle/pasture actions allowed by policy but cannot exploit field sync to invoke finance actions because arbitrary `{screenId,action}` is not accepted.
...
- Modify: `tests/e2e/p1-complete.spec.mjs` to reflect the expanded P1 rather than the earlier P1 milestone

**E2E journeys:**

1. Finance: create title -> partial settlement -> full settlement or reversal -> verify status/summary.
...
- [ ] Preserve existing dashboard, reports, IoT and P1 milestone E2E assertions.
...
- Modify: `tests/api-contract-gate.test.js` if present
...
- Modify: `README.md` only if it currently serves as product entry documentation
...
- [ ] Run IoT P0/P1 focused tests/workflow-equivalent commands to ensure field RFID work did not regress optional hardware.
...
- [ ] Wait for actual GitHub Actions results. Inspect failed job logs if any; do not infer success from local checks alone.
- [ ] Require hardening/QA and IoT workflows relevant to the branch to complete successfully.
- [ ] Merge only when PR is mergeable and required verification is green.

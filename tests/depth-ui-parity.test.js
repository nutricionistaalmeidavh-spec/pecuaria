...
test('existing modules expose pasture, reproduction, sanitary, field, reporting, intelligence and commercial-finance depth',async()=>{
  const [main,depth,depthBase,operations]=await Promise.all([source('../web/main.jsx'),source('../web/depth-components.jsx'),source('../web/depth-components-base.jsx'),source('../web/depth-operations.jsx')]);
  const combined=`${main}\n${depth}\n${depthBase}\n${operations}`;

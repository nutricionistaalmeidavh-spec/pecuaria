...
export function createCattlePresentation({persistence,localRuntime=null,recovery=null,capabilities=[],audit:providedAudit=null,iot:providedIoT=null,iotRuntime=null}={}){
  ...
  async function applySanitary(input,{batch=false}={}){
    ...
    const write=async store=>{
      ...
      for(let index=0;index<animals.length;index+=1){
        ...
        const event=recordSanitaryEvent({
          ...input,
          id:eventId,
          animalId:animal.id,
          ...
          nextDueAt,
          withdrawalUntil,
          productBatch:itemRecord?.payload?.batch??input?.productBatch??null,
          activeIngredient:protocol.activeIngredient??input?.activeIngredient??null,
          costMinor:costPerAnimalMinor
        });
        const saved=await scopedRepos.events.save({...event,kind:'sanitary'},{expectedVersion:0});
        results.push(saved);
        if(itemRecord){
          await scopedP1.inventoryMovements.save(createInventoryMovement({
            id:`sanitary-${eventId}`,
            itemId:productItemId,
            type:'out',
            quantity:dose,
            occurredAt,
            reason:`Sanitary application ${eventId}`,
            referenceType:'sanitary',
            referenceId:eventId,
            unitCostMinor
          }),{expectedVersion:0});
        }
        if(costPerAnimalMinor>0&&animal.lotId){
          await scopedFinance.save(createCattleCost({
            id:`${eventId}:finance`,
            lotId:animal.lotId,
            amountMinor:costPerAnimalMinor,
            description:`Sanitary application ${eventId}`,
            category:'sanitary',
            metadata:{sanitaryEventId:eventId,animalId:animal.id,productItemId,productBatch:itemRecord?.payload?.batch??null}
          }),{expectedVersion:0});
        }
      }

      if(itemRecord){
        await scopedP1.inventory.save({...itemRecord.payload,quantity:Number(itemRecord.payload.quantity)-totalQuantity},{expectedVersion:itemRecord.version});
      }
  ...
  const screens={
    ...
    sanitary:{kind:'sanitary-workspace',async load(){const[protocols,events]=await Promise.all([repos.sanitaryProtocols.list(),repos.events.list()]);return{protocols,events:events.filter(record=>record.payload.kind==='sanitary')}} ,actions:{
      saveProtocol:audited('cattle.sanitary.protocol.save','sanitary-protocol',(input,options)=>repos.sanitaryProtocols.save(createSanitaryProtocol(input),options??{})),
      record:audited('cattle.sanitary.record','sanitary-event',input=>applySanitary(input)),
      batchRecord:audited('cattle.sanitary.batch-record','sanitary-event',input=>applySanitary(input,{batch:true}))
    }},
    ...
    data:{kind:'data-tools',async load(){const [farms,breeds,categories,parties]=await Promise.all([repos.farmUnits.list(),repos.breeds.list(),repos.categories.list(),repos.parties.list()]);return{rows:[...farms,...breeds,...categories,...parties],parties};},actions:{
      ...
      saveParty:audited('cattle.party.save','party',(input,options)=>repos.parties.save(createCattleParty(input),options??{})),
    ...
    iot:{kind:'iot-devices',load:()=>iot.load(),actions:{
